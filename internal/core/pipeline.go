...
func (p *Pipeline) RunOnce(ctx context.Context) error {
	...
	for _, src := range p.Sources {
		...
		case "rss":
			...
			if err != nil { p.DB.LogError(ctx,"fetch:rss", src.Name+" : "+err.Error()); continue }

			for _, it := range items {
				if it.PublishedAt.Before(cutoff) { continue }
...
func (p *Pipeline) scoreItem(text string, sourceWeight float64) float64 {
	...
	for _, kw := range p.Filters.Positive {
		if strings.Contains(t, strings.ToLower(kw)) { score += 0.1 }
	}
	for _, kw := range p.Filters.Negative {
		if strings.Contains(t, strings.ToLower(kw)) { score -= 0.2 }
	}
	if score < 0 { score = 0 }
	if score > 1 { score = 1 }
	return score
